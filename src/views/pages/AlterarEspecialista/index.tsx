...
const NovoEspecialista: React.FC = () => {
  ...
  const onSubmit = (data: FormData) => {
    ...
    api.put('/enderecos', {...newData, nome: `${getValues('nome')} ${getValues('sobrenome')}`})
      .then(
        response => {
  ...
  async function populateEspecialistaFields(especialistaData: IUserRegister) {

    setValue('nome', especialistaData?.nome.split(' ')[0])
    setValue('sobrenome', especialistaData?.nome.replace(`${especialistaData?.nome.split(' ')[0]} `, ''))
    setValue('especialidade', especialistaData?.profissao.profissao)
    setValue('email', especialistaData?.email)
    setValue('registro', especialistaData?.registro)
    setValue('celular', especialistaData?.celular)
    setValue('telefone', especialistaData?.telefone)
    setValue('cep', `${especialistaData.endereco?.cep}`)
    setValue('numero', `${especialistaData.endereco?.numero}`)
    setValue('logradouro', `${especialistaData.endereco?.logradouro}`)
    setValue('bairro', `${especialistaData.endereco?.bairro}`)
    setValue('cidade', `${especialistaData.endereco?.cidade}`)
    setValue('uf', `${especialistaData.endereco?.uf}`)
  }
